...
protocol NotesDatabasesProtocol {
...
class NotesDatabase: NotesDatabasesProtocol {
    ...
    func update(identifier: UUID, title: String, text:String?) throws {
        ...
        do {
            ...
            print("Error actualizando informacion: \(error.localizedDescription)")
            throw DatabaseError.errorUpdate
        }
    }
    ...
    func remove(identifier: UUID) throws {
        ...
        do {
            guard let remoteNote = try container.mainContext.fetch(fetchDescriptor).first else {
                throw DatabaseError.errorRemove
            }
            container.mainContext.delete(remoteNote)
            return try container.mainContext.save()
        } catch {
            print("Error actualizando informacion: \(error.localizedDescription)")
            throw DatabaseError.errorRemove
